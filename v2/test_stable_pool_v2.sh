...
#npx locklift run $NO_TRACE --script v2/scripts/5-deploy-test-pool.ts --pools='[["foo", "bar", "qwe"],["qwe","tst","coin"]]' --contract_name='DexStablePool'
#npx locklift run $NO_TRACE --script v2/scripts/5-deploy-test-pair.ts --pairs='[["tst", "FooBarQweLp"],["tst","coin"],["tst","foo"],["coin","foo"]]' --contract_name='DexPair'

...
program
  ...
  .option(
    ...
    "Old DexPool contract name",
  )
  .option(
    "-ncn, --new_contract_name <new_contract_name>",
    "New DexPool contract name",
  )
...
const loadPoolData = async (pool, contractName: string) => {
  ...
  data.root = (
    await pool.methods.getRoot({ answerId: 0 }).call()
  ).dex_root.toString();
  // data.vault = (await pool.methods.getVault({answerId: 0}).call()).dex_vault.toString();

  data.current_version = (
    await pool.methods.getVersion({ answerId: 0 }).call()
  ).version;
  data.platform_code = (
    await pool.methods.platform_code().call()
  ).platform_code;
  ...
  const token_wallets = await pool.methods
    .getTokenWallets({ answerId: 0 })
    .call();
  data.lp_wallet = token_wallets.lp.toString();
  data.token_wallets = token_wallets.token_wallets.map(wallet =>
    wallet.toString(),
  );
  ...
  const balances = (await pool.methods.getBalances({ answerId: 0 }).call())
    .value0;
  data.lp_supply = balances.lp_supply.toString();
  data.balances = balances.balances.map(bal => bal.toString());

  const fee_params = (await pool.methods.getFeeParams({ answerId: 0 }).call())
    .value0;
  ...
  data.pool_type = Number(
    (await pool.methods.getPoolType({ answerId: 0 }).call()).value0,
  );
...
describe("Test Dex Pool contract upgrade", async function () {
  ...
  before("Load contracts", async function () {
    ...
    dexPool = migration.loadContract(
    ...
    targetVersion = (
      await dexRoot.methods
        .getPoolVersion({ answerId: 0, pool_type: options.pool_type })
        .call()
    ).value0;

    tokenRoots = {};
    for (let item of options.roots) {
      tokenRoots[item] = migration.loadContract(
        "TokenRootUpgradeable",
        tokens[item].symbol + "Root",
      );
    }

    oldPoolData = await loadPoolData(dexPool, options.old_contract_name);
    logger.log(
      `Old Pool(${dexPool.address}) data:\n${JSON.stringify(
        oldPoolData,
        null,
        4,
      )}`,
    ...
    newPoolData = await loadPoolData(
      NewVersionContract,
      options.new_contract_name,
    );
    logger.log(
      `New Pool(${NewVersionContract.address}) data:\n${JSON.stringify(
        newPoolData,
        null,
        4,
      )}`,
  ...
  describe("Check DexPool after upgrade", async function () {
    it("Check All data correct installed in new contract", async function () {
      ...
      expect(newPoolData.current_version.toString()).to.equal(
        targetVersion.toString(),
        "New current_version value incorrect",
      );
      expect(newPoolData.pool_type.toString()).to.equal(
        options.pool_type,
        "New current_version value incorrect",
      );
      ...
      for (let i = 0; i < N_COINS; i++) {
        expect(newPoolData.roots[i]).to.equal(
          oldPoolData.roots[i],
          `New ${tokens[options.roots[i]].symbol} root value incorrect`,
        );
      }
      ...
      for (let i = 0; i < N_COINS; i++) {
        expect(newPoolData.token_wallets[i]).to.equal(
          oldPoolData.token_wallets[i],
          `New ${tokens[options.roots[i]].symbol} wallet value incorrect`,
        );
      }
      // for (let i = 0; i < N_COINS; i++) {
      ...
      for (let i = 0; i < N_COINS; i++) {
        expect(newPoolData.balances[i]).to.equal(
          oldPoolData.balances[i],
          `New ${tokens[options.roots[i]].symbol} balance value incorrect`,
        );
      }

      expect(newPoolData.fee_pool).to.equal(
        oldPoolData.fee_pool,
        "New fee_pool value incorrect",
      );

      expect(newPoolData.fee_beneficiary).to.equal(
        oldPoolData.fee_beneficiary,
        "New fee_beneficiary value incorrect",
      );

      expect(newPoolData.fee_beneficiary_address).to.equal(
        oldPoolData.fee_beneficiary_address,
        "New fee beneficiary value incorrect",
      );

      expect(newPoolData.fee_referrer).to.equal(
        oldPoolData.fee_referrer,

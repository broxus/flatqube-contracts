...
// const N_COINS = 2;
//
// async function dexAccountBalancesV2(account) {
...
//
//     return {
//         balances: IS_FOO_LEFT ? [foo, bar] : [bar, foo],
//         walletBalances: IS_FOO_LEFT ? [walletFoo, walletBar] : [walletBar, walletFoo],
//         lp,
//         walletLp
//     };
// }
//
//
// function logBalances(label, a, d2, d3, pair, dex) {
//
//     const tokens = IS_FOO_LEFT ? [Constants.tokens.foo, Constants.tokens.bar] : [Constants.tokens.bar, Constants.tokens.foo];
//
//     if (d2) {
//         logger.log(`DexAccount#2 balance ${label}: ` +
//             `${d2.balances[0]} ${tokens[0].symbol}, ` +
//             `${d2.balances[1]} ${tokens[1].symbol}, ` +
//             `${d2.lp} LP`);
//     }
//     if (d3) {
//         logger.log(`DexAccount#3 balance ${label}: ` +
//             `${d3.balances[0]} ${tokens[0].symbol}, ` +
//             `${d3.balances[1]} ${tokens[1].symbol}, ` +
//             `${d3.lp} LP`);
//     }
//     if (a) {
//         logger.log(`Account#3 balance ${label}: ` +
//             `${a.balances[0]} ${tokens[0].symbol}, ` +
//             `${a.balances[1]} ${tokens[1].symbol}, ` +
//             `${a.lp} LP, ` +
//             `${a.ever} EVER`);
//     }
//     if (pair) {
//         logger.log(`DexPair ${label}: ` +
//             `${pair.balances[0]} ${tokens[0].symbol}, ` +
//             `${pair.balances[1]} ${tokens[1].symbol}, \n` +
//             `Accumulated fees: \n` +
//             `${pair.balances[0]} ${tokens[0].symbol}, ` +
//             `${pair.balances[1]} ${tokens[1].symbol}, \n` +
...
//     }
//     if (dex) {
//         logger.log(`DexVault#3 balance ${label}: ` +
//             `${a.balances[0]} ${tokens[0].symbol}, ` +
//             `${a.balances[1]} ${tokens[1].symbol}, ` +
//             `${a.lp} LP`);
//     }
// }
//
// async function dexPairInfoV2(tokens) {
...
//     const accumulated_fees = (await DexPairFooBar.call({method: 'getAccumulatedFees', params: {}})).map((e, i) => {
//         return new BigNumber(e).shiftedBy(-tokens[i].decimals).toString()
//     });
//     return {
//         balances: [
//             new BigNumber(balances.left_balance).shiftedBy(-tokens[0].decimals).toString(),
//             new BigNumber(balances.right_balance).shiftedBy(-tokens[1].decimals).toString()
//         ],
//         accumulated_fees,
...
//     }).catch(e => {/*ignored*/});
//     const ever = await locklift.utils.convertCrystal((await locklift.ton.getBalance(Account2.address)), 'ton').toNumber();
//     return {
//         balances: IS_FOO_LEFT ? [foo, bar] : [bar, foo],
//         lp,
//         ever
...
//         options.fee.threshold[FooRoot.address] = new BigNumber(1).shiftedBy(Constants.tokens.foo.decimals).toString();
//         options.fee.threshold[BarRoot.address] = new BigNumber(1).shiftedBy(Constants.tokens.bar.decimals).toString();
//
...
//             const dexAccount2Start = await dexAccountBalancesV2(DexAccount2);
//             const dexAccount3Start = await dexAccountBalancesV2(DexAccount3);
//             const dexPairInfoStart = await dexPairInfoV2();
//
//             logBalances('start', null, dexAccount2Start, dexAccount3Start, dexPairInfoStart, null);
//
...
//                 params: {
//                     amounts: [LEFT_AMOUNT, RIGHT_AMOUNT]
//                 }
//             });
//
//             const LP_REWARD = new BigNumber(expected.lp_reward).shiftedBy(-9).toString();
//
...
//             displayTx(tx);
//
//             const dexAccount2End = await dexAccountBalancesV2(DexAccount2);
//             const dexAccount3End = await dexAccountBalancesV2(DexAccount3);
//             const dexPairInfoEnd = await dexPairInfoV2(tokens);
//
//             logBalances('end', null, dexAccount2End, dexAccount3End, dexPairInfoEnd, null);
//
//             await migration.logGas();
//
...
//             const dexAccount3Start = await dexAccountBalancesV2(DexAccount3);
//
//             logBalances('start', accountStart, null, dexAccount3Start, dexPairInfoStart, dexStart);
//
...
//             const dexPairInfoEnd = await dexPairInfoV2();
//             const dexAccount3End = await dexAccountBalancesV2(DexAccount3);
//
//             logBalances('end', accountEnd, null, dexAccount3Start, dexPairInfoStart, dexEnd);
//
//             await migration.logGas();
//
//             let expectedBeneficiary = new BigNumber(expected.step_2_spent)
...
//             const dexPairInfoEnd = await dexPairInfoV2();
//
//             logBalances('end', accountEnd, null, dexAccount3End, dexPairInfoEnd, dexEnd);
//
//             await migration.logGas();
//
//             let expectedBeneficiary = new BigNumber(TOKENS_TO_EXCHANGE)

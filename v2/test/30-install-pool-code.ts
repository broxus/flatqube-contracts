...
import { Migration, Constants } from "../../utils/oldUtils/migration";
...
describe("Test Dex Pool contract upgrade", async function () {
  ...
  describe("Install DexPool code", async function () {
    it("Check code version", async function () {
      ...
      logger.log(
        `Installing new DexPool contract in DexRoot: ${dexRoot.address}`,
      );
      await dexRoot.methods
        .installOrUpdatePoolCode({
      ...
      expect(new BigNumber(startVersion).plus(1).toString()).to.equal(
        ...
        "DexPool code version incorrect",
      );
    });
  });
});

...
import { MSWrapper, PayloadOrder } from "../../utils/oldUtils/wrappers/ms";
...
describe("MultiScatter", () => {
  ...
  before("Deploy and load new migration", async () => {
    ...
    const addressMS = await multiScatterMigration(1, true, account1);

    multiScatter = await MSWrapper.from_addr(addressMS.address, account1);
  ...
  describe("Test MS", async () => {
    it("Create 2 order: full filled and part filled", async () => {
      ...
      const cancelPayload1 = await order1.originalPayloadCancel(
        22,
        111,
        account4.address,
      );
      const payloadLO1 = await order1.buildPayload(
        1,
        0.1,
        account4.address,
        "",
      ...
      const cancelPayload2 = await order2.originalPayloadCancel(
        22,
        222,
        account4.address,
      );
      const payloadLO2 = await order2.buildPayload(
        2,
        0.1,
        account4.address,
        "",
      ...
      const expectedAccount3Bar = new BigNumber(balanceBarAcc3Start.token || 0)
        .minus(new BigNumber(TOKEN_FOR_SPENT1 + TOKEN_FOR_SPENT2))
        .toString();
      const expectedAccount3Tst = new BigNumber(balanceTstAcc3Start.token || 0)
        .plus(new BigNumber(FULL_TOKEN_RECEIVE1 + FULL_TOKEN_RECEIVE2))
        .toString();

      const expectedAccount4Bar = new BigNumber(balanceBarAcc4Start.token || 0)
        .plus(new BigNumber(TOKEN_FOR_SPENT1 + FULL_TOKEN_SPENT2))
        .toString();
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
        .minus(new BigNumber(FULL_TOKEN_RECEIVE1 + FULL_TOKEN_RECEIVE2))
        .toString();
      ...
      ).to.above(0, "Wrong Order 2 Ever balance");
    ...
    it("Upgrade MultiScatter", async () => {
      const newCode = (
        await locklift.factory.getContractArtifacts("TestNewMultiScatter")
      ).code;
      const newVersion = 2;
      await multiScatter.upgrade(newCode, newVersion, account1.address);

      const newMS = await locklift.factory.getDeployedContract(
        "TestNewMultiScatter",
        multiScatter.address,
      );
      const currentVersion = (
        await newMS.methods.getDetails({ answerId: 1 }).call()
      ).value2;
    ...
    it("SendGas", async () => {
      const EverAccountStart = new BigNumber(
        await locklift.provider.getBalance(account3.address),
      )
        .shiftedBy(-9)
        .toString();
      const EverMultiScatterStart = new BigNumber(
        await locklift.provider.getBalance(multiScatter.address),
      )
        .shiftedBy(-9)
        .toString();
      ...
      const EverAccountEnd = new BigNumber(
        await locklift.provider.getBalance(account3.address),
      )
        .shiftedBy(-9)
        .toString();
      const EverMultiScatterEnd = new BigNumber(
        await locklift.provider.getBalance(multiScatter.address),
      )
        .shiftedBy(-9)
        .toString();

      expect(Number(EverAccountEnd)).to.above(
        Number(EverAccountStart),
        "Wrong account3 ever balance",
      );
      expect(Number(EverMultiScatterStart)).to.above(
        Number(EverMultiScatterEnd),
        "Wrong multiscatter ever balance",
      );
    });
  });
});

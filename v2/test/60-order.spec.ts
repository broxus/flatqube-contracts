...
} from "../../utils/oldUtils/migration.new.utils";

import { TokenWallet } from "../../utils/oldUtils/wrappers/tokenWallet";

import { OrderFactory } from "../../utils/oldUtils/wrappers/order_factory";
import { OrderRoot } from "../../utils/oldUtils/wrappers/order_root";
import { OrderWrapper } from "../../utils/oldUtils/wrappers/order";
...
describe("OrderTest", () => {
  ...
  before("deploy and load new migrations", async () => {
    ...
    const factoryAddress = (
      await rootTokenReceive.methods
        .walletOf({
          walletOwner: factoryOrder.address,
          answerId: 0,
        })
        .call()
    ).value0;
    ...
    const lproot = await dexPair.methods
      .getTokenRoots({
        answerId: 1,
      })
      .call();
    FooBarLpRoot = await locklift.factory.getDeployedContract(
    ...
    const dataBar = await dexAccount.methods
      .getWalletData({ token_root: rootTokenBar.address, answerId: 1 })
      .call();
    const dataTST = await dexAccount.methods
      .getWalletData({ token_root: rootTokenReceive.address, answerId: 1 })
      .call();
  ...
  describe("Direct execution Order", async () => {
    it("Check full execution with 2 buyer, case 1.1", async () => {
      ...
      const payload = await RootOrderBar.buildPayloadRoot(
        ...
        numberString(TOKENS_TO_EXCHANGE2, tstDecimals),
        0,
        0,
      );
      ...
      const { traceTree: ms1 } = await locklift.tracing.trace(
        tstWallet4.transfer(
          numberString(TOKENS_TO_EXCHANGE2_ACC3, tstDecimals),
          order.address,
          payloadLO,
          toNano(5),
        ),
      ...
      await ms1?.beautyPrint();

      for (let addr in ms1?.balanceChangeInfo) {
        console.log(
          addr +
            ": " +
            ms1?.balanceChangeInfo[addr].balanceDiff.shiftedBy(-9).toString(),
        );
      }

      const { traceTree: ms2 } = await locklift.tracing.trace(
        tstWallet5.transfer(
          numberString(TOKENS_TO_EXCHANGE2_ACC4, tstDecimals),
          order.address,
          payloadLO,
          toNano(5),
        ),
      ...
      await ms2?.beautyPrint();

      for (let addr in ms2?.balanceChangeInfo) {
        console.log(
          addr +
            ": " +
            ms2?.balanceChangeInfo[addr].balanceDiff.shiftedBy(-9).toString(),
        );
      }
      ...
      const balanceTstAcc5End = await accountTokenBalances(
        tstWallet5,
        tstDecimals,
      );
      const balanceBarAcc5End = await accountTokenBalances(
        barWallet5,
        barDecimals,
      );
      await displayLog(balanceBarAcc5End, balanceTstAcc5End, false, "Account5");

      const expectedAccount3Bar = new BigNumber(balanceBarAcc3Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE1))
        .toString();
      const expectedAccount3Tst = new BigNumber(balanceTstAcc3Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE2))
        .toString();
      const expectedAccount4Bar = new BigNumber(balanceBarAcc4Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE1_ACC3))
        .toString();
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC3))
        .toString();
      const expectedAccount5Bar = new BigNumber(balanceBarAcc5Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE1_ACC4))
        .toString();
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC4))
        .toString();

      // @ts-ignore
      expect(3).to.be.equal(
        Number(await order.status()),
        "Wrong status Limit Order",
      );
      expect(0).to.be.equal(
        Number(await locklift.provider.getBalance(order.address)),
        "Wrong Balance Ever Limit Order",
      );
      ...
      expect(expectedAccount5Bar).to.equal(
        balanceBarAcc5End.token.toString(),
        "Wrong Account5 Bar balance",
      );
      expect(expectedAccount5Tst).to.equal(
        balanceTstAcc5End.token.toString(),
        "Wrong Account5 Tst balance",
      );
    });
    it("Check partial execution Order, case 2.1", async () => {
      ...
      const balanceBarAcc6Start = await accountTokenBalances(
        barWallet6,
        barDecimals,
      );
      const balanceTstAcc6Start = await accountTokenBalances(
        tstWallet6,
        tstDecimals,
      );
      await displayLog(
      ...
      const { traceTree: ms3 } = await locklift.tracing.trace(
        tstWallet6.transfer(
          numberString(TOKENS_TO_EXCHANGE2_ACC5, tstDecimals),
          order.address,
          payloadLO,
          toNano(5),
        ),
      ...
      await ms3?.beautyPrint();

      for (let addr in ms3?.balanceChangeInfo) {
        console.log(
          addr +
            ": " +
            ms3?.balanceChangeInfo[addr].balanceDiff.shiftedBy(-9).toString(),
        );
      }
      ...
      const balanceBarAcc6End = await accountTokenBalances(
        barWallet6,
        barDecimals,
      );
      const balanceTstAcc6End = await accountTokenBalances(
        tstWallet6,
        tstDecimals,
      );
      ...
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC4))
        .toString();
      const expectedAccount6Bar = new BigNumber(balanceBarAcc6Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE1_ACC5))
        .toString();
      const expectedAccount6Tst = new BigNumber(balanceTstAcc6Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC5))
        .toString();
      ...
      expect(expectedAccount6Bar).to.equal(
        balanceBarAcc6End.token.toString(),
        "Wrong Account6 Bar balance",
      );
      expect(expectedAccount6Tst).to.equal(
        balanceTstAcc6End.token.toString(),
        "Wrong Account6 Tst balance",
      );
    });
    it("Check partial execution Order, case 2.2", async () => {
      ...
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        ...
        .toString();
      const expectedAccount6Bar = new BigNumber(balanceBarAcc6Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE1_ACC5 - 5))
        .toString();
      const expectedAccount6Tst = new BigNumber(balanceTstAcc6Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC5 - 10))
        .toString();
    ...
    it("Check create order and closed, case 3.1", async () => {
      ...
      const { traceTree } = await order.cancel();
      ...
      for (let addr in traceTree?.balanceChangeInfo) {
        console.log(
          addr +
            ": " +
            traceTree?.balanceChangeInfo[addr].balanceDiff
              .shiftedBy(-9)
              .toString(),
        );
      }
      ...
      const expectedAccount3Bar = new BigNumber(balanceBarAcc3Start.token || 0)
        ...
        .toString();
      const expectedAccount4Bar = new BigNumber(balanceBarAcc4Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE1))
        .toString();
      ...
      expect(balanceTstAcc3Start.token.toString()).to.equal(
        balanceTstAcc3End.token.toString(),
        "Wrong Account3 Tst balance",
      );
      ...
      expect(balanceTstAcc4Start.token.toString()).to.equal(
        balanceTstAcc4End.token.toString(),
        "Wrong Account4 Tst balance",
      );
    ...
    it("Check create order, part exhcange and closed, case 3.2", async () => {
      ...
      const { traceTree: ms2 } = await order.cancel();
      ...
      const expectedAccount3Bar = new BigNumber(balanceBarAcc3Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE1 / 2))
        .toString();
      const expectedAccount3Tst = new BigNumber(balanceTstAcc3Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC3))
        .toString();
      const expectedAccount4Bar = new BigNumber(balanceBarAcc4Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE1 / 2))
        .toString();
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC3))
        .toString();
    ...
    it("Check execution closed order, case 4.1", async () => {
      ...
      const { traceTree: ms1 } = await order.cancel();
      ...
      expect(balanceBarAcc4Start.token.toString()).to.equal(
        balanceBarAcc4End.token.toString(),
        "Wrong Account4 Bar balance",
      );
      expect(balanceTstAcc4Start.token.toString()).to.equal(
    ...
    it("Check execution closed order BY BACKEND, case 4.2", async () => {
      ...
      const { traceTree: ms1 } = await order.backendCancel(signer, 15);
  ...
  describe("Execution order via DEX", async () => {
    it("Order from backend SUCCESS", async () => {
      ...
      console.log(
        `Expected fee: ${numberString(
          Number(expected.expected_fee),
          -barDecimals,
        )} BAR`,
      );
      console.log(
        `Expected receive amount: ${numberString(
          Number(expected.expected_amount),
          -tstDecimals,
        )} TST`,
      );
      ...
      const expectedAccount3Tst = new BigNumber(balanceTstAcc3Start.token || 0)
        .plus(new BigNumber(expected.expected_amount).shiftedBy(-tstDecimals))
        .toString();
    ...
    it("Order from user SUCCESS", async () => {
      ...
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
        .plus(
          new BigNumber(expected.expected_amount)
            .shiftedBy(-tstDecimals)
            .minus(new BigNumber(TOKENS_TO_EXCHANGE2)),
        )
        .toString();
  ...
  describe("Emergency mode", async () => {
    ...
    it("Emergency mode on, send TIP3, off (Bad OrderClosed contract)", async () => {
      ...
      const BadOrderCode = (
        await locklift.factory.getContractArtifacts("TestNewOrderBad")
      ).code;
      ...
      const balanceBarAcc3Proccess = await accountTokenBalances(
        barWallet3,
        barDecimals,
      );
      expect(balanceBarAcc3Proccess.token.toString()).to.equal(
        balanceBarAcc3Start.token.minus(TOKENS_TO_EXCHANGE1).toString(),
        "Wrong Account3 Bar balance",
      );
      const signer1 = await locklift.keystore.getSigner("1");
      await factoryOrder.setEmergency(
      ...
      const GasAccount3 = new BigNumber(
        await locklift.provider.getBalance(account3.address),
      ).shiftedBy(-9);
      const GasOrder = new BigNumber(
        await locklift.provider.getBalance(order.address),
      ).shiftedBy(-9);
      console.log(
        `GasAccount3 - ${GasAccount3.toString()}\nGasOrder - ${GasOrder.toString()}`,
      );

      await order.sendGas(
        account3.address,
        toNano(GasOrder.minus(1).toString()),
        66,
        signer1,
      );
      await factoryOrder.setEmergency(
      ...
      const GasAccount3End = new BigNumber(
        await locklift.provider.getBalance(account3.address),
      ).shiftedBy(-9);
      ...
      expect(
        GasAccount3End.minus(GasAccount3)
          .plus(0.3)
          .isGreaterThanOrEqualTo(GasOrder.minus(1)),
      ).to.equal(true, "Wrong gas Balance");
      // @ts-ignore
      expect(balanceBarAcc3Start.token.toString()).to.equal(
        balanceBarAcc3End.token.toString(),
        "Wrong Account3 Bar balance",
    ...
    it("Test proxyTokensTransfer and sendGas on OrderRoot", async () => {
      ...
      const walletRootBarAddress = await rootTokenBar.methods
        .walletOf({
          answerId: 0,
          walletOwner: RootOrderBar.address,
        })
        .call();
      const walletRootBar = await TokenWallet.from_addr(
        walletRootBarAddress.value0,
        factoryOrder.address,
        "factoryWalletTst",
      );
      ...
      const mainCode = (
        await locklift.factory.getContractArtifacts("OrderRoot")
      ).code;
      const testFactoryCode = (
        await locklift.factory.getContractArtifacts("TestNewOrderRoot")
      ).code;
      ...
      const newRoot = await locklift.factory.getDeployedContract(
        "TestNewOrderRoot",
        RootOrderBar.address,
      );
      // Проверяем
      ...
      const barWallet8Address = await deployWallet(
        account8,
        rootTokenBar,
        account1,
      );
      const barWallet8 = await TokenWallet.from_addr(
        barWallet8Address,
        account6,
        "barWallet8",
      );
      ...
      await locklift.tracing.trace(
        factoryOrder.contract.methods
          .proxyRootTokensTransfer({
            _notify: true,
      ...
      expect(
        tokenRootStart
          .minus(tokenRootEnd)
          .isGreaterThanOrEqualTo(new BigNumber(numberString(50, barDecimals))),
      ).to.equal(true, "Wrong gas Balance");
      expect(
        tokenAccountEnd
          .minus(tokenAccountStart)
          .isGreaterThanOrEqualTo(new BigNumber(numberString(50, barDecimals))),
      ).to.equal(true, "Wrong gas Balance");
    });
  });
  describe("Matching orders", async () => {
    it("Matching on full filled 1", async () => {
      ...
      expect(Number(feeParams.params.matchingNumerator)).to.equal(
        1,
        "Wrong MATCHINGNUMERATOR",
      );
      expect(Number(feeParams.params.matchingDenominator)).to.equal(
        2,
        "Wrong MATCHINGDENOMINATOR",
      );
      ...
      const comissionAmount =
        rewardAmount *
        (Number(feeParams.params.matchingNumerator) /
          Number(feeParams.params.matchingDenominator));

      const expectedFactoryTstWallet = new BigNumber(
        balanceFactoryTstStart.token || 0,
      )
        .plus(new BigNumber(comissionAmount))
        .toString();
      ...
      const expectedAccount5Bar = new BigNumber(
        balanceBarAcc5Start.token || 0,
      ).toString();
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        .plus(new BigNumber(rewardAmount - comissionAmount))
        .toString();
    ...
    it("Matching on full filled 2", async () => {
      ...
      const expectedAccount5Tst = new BigNumber(
        balanceTstAcc5Start.token || 0,
      ).toString();
      ...
      expect(balanceFactoryTstStart.token.toString()).to.equal(
        balanceFactoryTstEnd.token.toString(),
        "Wrong Factory Tst balance",
      );
      expect(expectedAccount3Bar).to.equal(
    ...
    it("Matching on different filled: order 1 - full and 2 - part 1", async () => {
      ...
      const rewardAmount =
        TOKENS_TO_EXCHANGE_RECEIVE1 *
        (TOKENS_TO_EXCHANGE_SPENT1 / TOKENS_TO_EXCHANGE_RECEIVE1 -
          TOKENS_TO_EXCHANGE_RECEIVE2 / TOKENS_TO_EXCHANGE_SPENT2);
      const comissionAmount =
        rewardAmount *
        (Number(feeParams.params.matchingNumerator) /
          Number(feeParams.params.matchingDenominator));
      ...
      expect(
        Number(await locklift.provider.getBalance(order2.address)),
      ).to.be.above(0, "Wrong Order Ever balance");
      expect(expectedFactoryTstWallet).to.equal(
        balanceFactoryTstEnd.token.toString(),
        "Wrong Factory Tst balance",
      );
    ...
    it("Matching on different filled: order 1 - full and 2 - part 2", async () => {
      ...
      const rewardAmount = (TOKENS_TO_EXCHANGE_SPENT1 - expectedRecive).toFixed(
        9,
      );
      const comissionAmount =
        Number(rewardAmount) *
      ...
      const expectedFactoryTstWallet = new BigNumber(
        ...
        .plus(new BigNumber(comissionAmount).toFixed(9))
        .toString();
      ...
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        .plus(
          new BigNumber(Number(rewardAmount) - comissionAmount).toFixed(9, 1),
        )
        .toString();
    ...
    it("Matching on different filled: order 1 - part and 2 - full", async () => {
      ...
      const expectedAccount3Bar = new BigNumber(balanceBarAcc3Start.token || 0)
        .plus(new BigNumber(TOKENS_TO_EXCHANGE_SPENT2))
        .toString();
      const expectedAccount3Tst = new BigNumber(balanceTstAcc3Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE_SPENT1))
        .toString();
      ...
      const LO1TstWallet = await TokenWallet.from_addr(
        addressTst,
        order1.address,
      );
      let balanceLimitOrder1 = numberString(
        Number(await LO1TstWallet.balance()),
        tstDecimals,
      );
      const expectedLO1Tst = numberString(
        TOKENS_TO_EXCHANGE_SPENT1 -
          TOKENS_TO_EXCHANGE_SPENT2 *
            (TOKENS_TO_EXCHANGE_SPENT1 / TOKENS_TO_EXCHANGE_RECEIVE1),
        tstDecimals,
      ...
      expect(
        Number(await locklift.provider.getBalance(order1.address)),
      ).to.be.above(0, "Order 1 ever balance should be higher 0!");
      expect(0).to.equal(
        Number(await locklift.provider.getBalance(order2.address)),
        "Wrong Order 2 Ever balance",
      );
      expect(expectedFactoryTstWallet).to.equal(
        new BigNumber(balanceFactoryTstEnd.token || 0).toString(),
        "Wrong Factory Tst balance",
      );
      expect(expectedAccount3Bar).to.equal(
        balanceBarAcc3End.token.toString(),
      ...
      expect(expectedLO1Tst).to.equal(
        new BigNumber(balanceLimitOrder1).shiftedBy(-tstDecimals).toString(),
        "Wrong Limit Order 1 Tst wallet balance",
      );
    });
    ...
    it("Matching on different filled: order 1 - full and 2 - full order from backend 3", async () => {
      ...
      const expectedAccount4Bar = new BigNumber(balanceBarAcc4Start.token || 0)
        ...
        .toString();
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
        .plus(new BigNumber(expectedReceive))
        .toString();
    ...
    it("Matching on different filled: order 1 - part and 2 - full order from backend 4", async () => {
      ...
      const rewardAmount =
        (TOKENS_TO_EXCHANGE_SPENT2 / TOKENS_TO_EXCHANGE_RECEIVE1) *
          TOKENS_TO_EXCHANGE_SPENT1 -
        (TOKENS_TO_EXCHANGE_SPENT2 / TOKENS_TO_EXCHANGE_SPENT2) *
          TOKENS_TO_EXCHANGE_RECEIVE2;
      ...
      expect(
        Number(await locklift.provider.getBalance(order1.address)),
      ).to.above(0, "Wrong Order Ever balance 1");
      expect(0).to.equal(
        Number(await locklift.provider.getBalance(order2.address)),
        "Wrong Order Ever balance 2",
      );
    ...
    it("Matching on different filled: order 1 - full and 2 - part order from backend 5", async () => {
      ...
      const feeParams = await RootOrderTst.feeParams();
      const rewardAmount =
        (TOKENS_TO_EXCHANGE_RECEIVE1 / TOKENS_TO_EXCHANGE_RECEIVE1) *
          TOKENS_TO_EXCHANGE_SPENT1 -
        (TOKENS_TO_EXCHANGE_RECEIVE1 / TOKENS_TO_EXCHANGE_SPENT2) *
          TOKENS_TO_EXCHANGE_RECEIVE2;
      let comissionAmount = 0;
      if (
        Number(feeParams.params.matchingNumerator) != 0 &&
        Number(feeParams.params.matchingDenominator) != 0
      ) {
        comissionAmount =
          rewardAmount *
          (Number(feeParams.params.matchingNumerator) /
    ...
    it("Matching order failure", async () => {
      ...
      expect(balanceBarAcc5Start.token.toString()).to.equal(
        balanceBarAcc5End.token.toString(),
        "Wrong Account5 Bar balance",
      );
      expect(balanceTstAcc5Start.token.toString()).to.equal(
        balanceTstAcc5End.token.toString(),
        "Wrong Account5 Tst balance",
      );
    });
  ...
  describe("Check callbacks and result", async () => {
    it("Order part filled with 1 buyers and get successPayloads", async () => {
      ...
      const successPayload4 = await order.originalPayloadSuccess(
        200,
        555,
        account3.address,
      );
      const cancelPayload4 = await order.originalPayloadCancel(
        201,
        555,
        account4.address,
      );
      const payloadLO4 = await order.buildPayload(
        1,
        0.1,
        account4.address,
        successPayload4,
      ...
      const expectPayload = await order.buildSuccessPayload(
        50,
        successPayload4,
        account4.address,
      );
      ...
      expect(depositCalls[0].payload.toString()).to.eq(
        expectPayload.toString(),
        "Fault payload success",
      );
      expect(traceTree)
        .to.call("acceptTransfer", barWallet4.address)
        .withNamedArgs({
          amount: "5000000000",
    ...
    it("Order part filled with 1 buyers and get cancelPayloads", async () => {
      ...
      const cancelPayload4 = await order.originalPayloadSuccess(
        201,
        666,
        account4.address,
      );
      ...
      const { traceTree } = await locklift.tracing.trace(
        barWallet4.transfer(
          numberString(TOKENS_TO_EXCHANGE2_ACC3, barDecimals),
          order.address,
          payloadLO4,
          toNano(5),
        ),
      ...
      expect(balanceTstAcc3End.token.toString()).to.equal(
        balanceTstAcc3End.token.toString(),
        "Wrong Account3 Tst balance",
      );
      expect(balanceBarAcc4End.token.toString()).to.equal(
        balanceBarAcc4End.token.toString(),
        "Wrong Account4 Bar balance",
      );
      expect(balanceTstAcc4End.token.toString()).to.equal(
        balanceTstAcc4End.token.toString(),
        "Wrong Account4 Tst balance",
      );
    });
    it("Order root get cancelPayload from create order", async () => {
      ...
      const cancelPayloadRoot = await RootOrderTst.originalPayloadCancel(
        22,
        777,
        account3.address,
      );
      ...
      const { traceTree } = await locklift.tracing.trace(
        tstWallet3.transfer(
          numberString(TOKENS_TO_EXCHANGE1, tstDecimals),
          RootOrderBar.address,
          payload,
          toNano(6),
        ),
      ...
      const expectPayload = await RootOrderBar.buildCancelPayload(
        53,
        201,
        cancelPayloadRoot,
      );
      ...
      expect(traceTree)
        .to.call("acceptTransfer", tstWallet3.address)
        .withNamedArgs({
          amount: "10000000000",
      ...
      // @ts-ignore
      expect(balanceBarAcc3End.token.toString()).to.equal(
        balanceBarAcc3End.token.toString(),
        "Wrong Account3 Bar balance",
      );
      expect(balanceTstAcc3End.token.toString()).to.equal(
  ...
  describe("Fee params Order", async () => {
    it("Check fee execution, case 1.1", async () => {
      ...
      const balanceTstFactoryStart = await accountTokenBalances(
        FactoryWalletTst,
        tstDecimals,
      );
      ...
      expect(feeParams.params.numerator).to.equal(
        NUMERATOR.toString(),
        "Wrong NUMERATOR",
      );
      expect(feeParams.params.denominator).to.equal(
        DENOMINATOR.toString(),
        "Wrong DENOMINATOR",
      );
      expect(feeParams.params.matchingNumerator).to.equal(
        MATCHINGNUMERATOR.toString(),
        "Wrong MATCHINGNUMERATOR",
      );
      expect(feeParams.params.matchingDenominator).to.equal(
        MATCHINGDENOMINATOR.toString(),
        "Wrong MATCHINGDENOMINATOR",
      );
      ...
      const balanceTstFactoryEnd = await accountTokenBalances(
        FactoryWalletTst,
        tstDecimals,
      );
      await displayLog(0, balanceTstFactoryEnd, true, "Factory");
      const fees = new BigNumber(
        expectAmountFee(NUMERATOR, DENOMINATOR, TOKENS_TO_EXCHANGE2),
      );
      ...
      const expectedAccount4Bar = new BigNumber(balanceBarAcc4Start.token || 0)
        ...
        .minus(
          new BigNumber(
            expectAmountFee(NUMERATOR, DENOMINATOR, TOKENS_TO_EXCHANGE1_ACC3),
          ),
        )
        .toString();
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
      ...
      const expectedAccount5Bar = new BigNumber(balanceBarAcc5Start.token || 0)
        ...
        .minus(
          new BigNumber(
            expectAmountFee(NUMERATOR, DENOMINATOR, TOKENS_TO_EXCHANGE1_ACC4),
          ),
        )
        .toString();
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC4))
        .toString();
      const expectedTstFactory = new BigNumber(
        balanceTstFactoryStart.token || 0,
      )
        .plus(
          new BigNumber(
            expectAmountFee(NUMERATOR, DENOMINATOR, TOKENS_TO_EXCHANGE2),
      ...
      expect(expectedTstFactory).to.equal(
        balanceTstFactoryEnd.token.toString(),
        "Wrong Beneficiary balance",
      );
    });
    it("Check fee execution, case 1.2 (Change Fee parameters)", async () => {
      ...
      expect(expectedAccount3Bar).to.equal(
        balanceBarAcc3End.token.toFixed(8),
        "Wrong Account3 Bar balance",
      );
      expect(expectedAccount3Tst).to.equal(
        balanceTstAcc3End.token.toFixed(8),
        "Wrong Account3 Tst balance",
      );
      expect(expectedAccount4Bar).to.equal(
        balanceBarAcc4End.token.toFixed(8),
        "Wrong Account4 Bar balance",
      );
      expect(expectedAccount4Tst).to.equal(
        balanceTstAcc4End.token.toFixed(8),
        "Wrong Account4 Tst balance",
      );
      expect(expectedAccount5Bar).to.equal(
        balanceBarAcc5End.token.toFixed(8),
        "Wrong Account5 Bar balance",
      );
      expect(expectedAccount5Tst).to.equal(
        balanceTstAcc5End.token.toFixed(8),
        "Wrong Account5 Tst balance",
      );
      expect(expectedTstFactory).to.equal(
        balanceTstFactoryEnd.token.toFixed(8),
        "Wrong Beneficiary balance",
      );
    });
    it("Check fee execution, case 1.3 (Withdraw fee)", async () => {
      ...
      const balanceTstRecipientStart = await accountTokenBalances(
        tstWallet2,
        tstDecimals,
      );
      ...
      const balanceTstRecipientEnd = await accountTokenBalances(
        tstWallet2,
        tstDecimals,
      );
      ...
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        .minus(new BigNumber(TOKENS_TO_EXCHANGE2_ACC4))
        .toFixed(9);
      const expectedRecipientTst = new BigNumber(
        balanceTstRecipientStart.token || 0,
      )
        .plus(
          new BigNumber(
            expectAmountFee(NUMERATOR, DENOMINATOR, TOKENS_TO_EXCHANGE2),
      ...
      expect(expectedAccount3Bar).to.equal(
        balanceBarAcc3End.token.toFixed(9),
        "Wrong Account3 Bar balance",
      );
      expect(expectedAccount3Tst).to.equal(
        balanceTstAcc3End.token.toFixed(9),
        "Wrong Account3 Tst balance",
      );
      expect(expectedAccount4Bar).to.equal(
        balanceBarAcc4End.token.toFixed(9),
        "Wrong Account4 Bar balance",
      );
      expect(expectedAccount4Tst).to.equal(
        balanceTstAcc4End.token.toFixed(9),
        "Wrong Account4 Tst balance",
      );
      expect(expectedAccount5Bar).to.equal(
        balanceBarAcc5End.token.toFixed(9),
        "Wrong Account5 Bar balance",
      );
      expect(expectedAccount5Tst).to.equal(
        balanceTstAcc5End.token.toFixed(9),
        "Wrong Account5 Tst balance",
      );
      expect(expectedTstFactory).to.equal(
        balanceTstFactoryEnd.token.toFixed(9),
        "Wrong Beneficiary balance",
      );
      expect(expectedRecipientTst).to.equal(
        balanceTstRecipientEnd.token.toFixed(9),
    ...
    it("Check fee execution, case 1.4 (Set beneficiary)", async () => {
      ...
      await displayLog(
        await accountTokenBalances(barWallet3, barDecimals),
        await accountTokenBalances(tstWallet3, tstDecimals),
        true,
        "Account3",
      );
      ...
      const newBeneficiaryAddress = (
        await rootTokenReceive.methods
          .walletOf({
            walletOwner: newBeneficiary.address,
            answerId: 0,
          })
          .call()
      ).value0;
      const newBeneficiaryWalletTst = await TokenWallet.from_addr(
        newBeneficiaryAddress,
        newBeneficiary,
        "newBeneficiaryWalletTst",
      );
      await locklift.tracing.trace(
        barWallet3.transfer(
          numberString(TOKENS_TO_EXCHANGE1, barDecimals),
  ...
  describe("Upgrade Order contracts", async () => {
    it("Check Order upgrade", async () => {
      ...
      const payload = await RootOrderBar.buildPayloadRoot(
        ...
        zeroAddress,
        rootTokenBar.address,
        numberString(TOKENS_TO_EXCHANGE2, barDecimals),
        0,
        0,
      );
      ...
      const testOrderCode = (
        await locklift.factory.getContractArtifacts("TestNewOrder")
      ).code;
      ...
      const newOrder = await locklift.factory.getDeployedContract(
        "TestNewOrder",
        order.address,
      );
      const testMessage = (await newOrder.methods.newFunc().call()).value0;
      const newVersion = (
        await newOrder.methods.getDetails({ answerId: 1 }).call()
      ).value0.version;

      expect(testMessage).to.equal("New Order", "Wrong Upgrade OrderFactory");
      expect(newVersion.toString()).to.equal(
        NEW_VERSION.toString(),
        "Wrong Order new version",
      );
    });
    it("Check Order Root upgrade", async () => {
      ...
      const newVersion = (
        await newRoot.methods.getVersion({ answerId: 1 }).call()
      ).value0;

      expect(testMessage).to.equal(
        "New Order Root",
        "Wrong Upgrade OrderFactory",
      );
      expect(newVersion.toString()).to.equal(
        NEW_VERSION.toString(),
        "Wrong OrderFactory new version",
      );
    });
    it("Check Order Factory upgrade", async () => {
      ...
      const testFactoryCode = (
        await locklift.factory.getContractArtifacts("TestNewOrderFactory")
      ).code;
      const NEW_VERSION = 3;
      await factoryOrder.upgrade(
        testFactoryCode,
        account1.address,
        NEW_VERSION,
      );

      const newFactory = await locklift.factory.getDeployedContract(
        "TestNewOrderFactory",
        factoryOrder.address,
      );
      const testMessage = (await newFactory.methods.newFunc().call()).value0;
      const newVersion = (
        await newFactory.methods.getVersion({ answerId: 1 }).call()
      ).value0;
      const owner = (await newFactory.methods.getOwner({ answerId: 2 }).call())
        .value0;

...
import {Address, Contract, getRandomNonce, zeroAddress, toNano} from 'locklift';
...
describe('OrderTest', () => {
    ...
    const barDecimals = Constants.tokens.bar.decimals;
    const tstDecimals = Constants.tokens.tst.decimals;
    ...
    describe('Direct execution Order', async () => {
        it('Check full execution with 2 buyer, case 1.1', async () => {
            ...
            const payload = await RootOrderBar.buildPayloadRoot(
                0, rootTokenReceive.address, numberString(TOKENS_TO_EXCHANGE2, tstDecimals),
                toNano(0.1), 0, 0
            )
    ...
    describe('Matching orders', async  () => {
        it('Matching on full filled order', async () => {
            ...
            const payload1 = await RootOrderBar.buildPayloadRoot(
                0, rootTokenBar.address, numberString(TOKENS_TO_EXCHANGE_RECEIVE1, barDecimals),
                toNano(0.1), 0, 0
            )
            ...
            const payload2 = await RootOrderBar.buildPayloadRoot(
                0, rootTokenReceive.address, numberString(TOKENS_TO_EXCHANGE_RECEIVE2, tstDecimals),
                toNano(0.1), 0, 0
            )
    ...
    describe('Upgrade Order contracts', async () => {
        it('Check Order upgrade', async () => {
            ...
            const payload = await RootOrderBar.buildPayloadRoot(
                0, rootTokenBar.address, numberString(TOKENS_TO_EXCHANGE2, barDecimals),
                toNano(0.1), 0, 0
            )
...
function numberString(amount: number, decimals: number):string {
    return new BigNumber(amount).shiftedBy(Constants.tokens.bar.decimals).toString();
}

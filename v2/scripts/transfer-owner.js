...
const submitTransactionAcceptOwner = async (account, target, keyPair) => {
  return await account.run({
    ...
    params: {
      ...
      allBalance: false,
      payload: (
        await this.locklift.ton.client.abi.encode_message_body({
...
async function main() {
  ...
  const oldOwner = migration.load(
    await locklift.factory.getAccount("Wallet"),
    "Account1",
  );
  oldOwner.afterRun = afterRun;
  const Msig = await locklift.factory.getContract(
    "SafeMultisigWallet",
    "safemultisig",
  );
  ...
  const tokenFactory = migration.load(
    await locklift.factory.getContract("TokenFactory"),
    "TokenFactory",
  );
  console.log(
    `Transferring DEX ownership from ${oldOwner.address} to ${newOwner.address}`,
  );
  ...
  console.log(
    `transIds: ${await newOwner.call({ method: "getTransactionIds" })}`,
  );
}

...
async function main() {
  const USDT_USDC_LP_ROOT =
    '0:1ddd1a0a7d6ee3cef8ccb9e6aa02f5c142658522ddd40f21ae7160177ced0e12';
  ...
  const tokenRoot = await locklift.factory.getDeployedContract(
    ...
    new Address(USDT_USDC_LP_ROOT),
  );
  const tokenWalletAddress = (
    await tokenRoot.methods
      .walletOf({ answerId: 0, walletOwner: dexVault.address })
      .call()
  ).value0;

  console.log(
    `DexVault wallet address for FlatQube-LP-USDT-USDC: ${tokenWalletAddress.toString()}`,
  );

  const vaultWalletsStart = (await dexVault.methods._vaultWallets({}).call())
    ._vaultWallets;
  ...
  const vaultWalletsEnd = (await dexVault.methods._vaultWallets({}).call())
    ._vaultWallets;

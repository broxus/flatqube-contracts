...
const ORDER_CODE_HASH = 'e29feb6cc7c399d348f88a4c857fa858cd230a0c62ee1d54c52b0a604e66bd56';
...
async function main() {
    ...
    while (hasResults) {
        ...
        for (const orderAddress of result.accounts) {
            ...
            const dataFactory = (await Order.methods.getDetails({answerId: 0}).call({})).value0;

            if (dataFactory.factory.toString() == ORDER_FACTORY_ADDRESS && dataFactory.state.toString() == "2") {
                orderForUpdate.push({
                    order: orderAddress,
                });
            }

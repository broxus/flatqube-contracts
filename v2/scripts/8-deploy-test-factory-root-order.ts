...
async function main() {
    ...
    const FactoryOrder = await locklift.factory.getContractArtifacts('OrderFactory');
    ...
    const factoryOrder = (await locklift.factory.deployContract({
    ...
    const rootToken = await locklift.factory.getDeployedContract( 'TokenRootUpgradeable', migration.getAddress('BarRoot'));
    logMigrationProcess(
        'factoryOrder',
        'createOrderRoot',
        'Create OrderRoot...',
    );
    await factoryOrder.methods.createOrderRoot({token: rootToken.address, callbackId: 1}).send({

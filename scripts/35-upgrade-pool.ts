...
async function main() {
  ...
  const tokensSymbols = options.roots.map(
      (symbol: string) => Constants.tokens[symbol]
          ? Constants.tokens[symbol].symbol
          : symbol
  );

  const tokens: Contract<TokenRootUpgradeableAbi>[] = tokensSymbols.map(
    (symbol: string) => locklift.deployments.getContract(`token-${symbol}`),
  );
  const poolName =
    `DexStablePool_` +
      tokensSymbols.map((symbol: string) => `token-${symbol}`).join("_");

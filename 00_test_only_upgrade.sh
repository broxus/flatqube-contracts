...
export DEFAULT_PARAMS="--config locklift.config.js --disable-build --enable-tracing --network local"
...
npx locklift run $NO_TRACE --script scripts/3-mint-test-tokens.js --mints='[{"account":2,"amount":2000000000,"token":"bar"},{"account":2,"amount":2000000000,"token":"foo"},{"account":2,"amount":2000000000,"token":"qwe"}]'
...
npx locklift test $DEFAULT_PARAMS --tests test/10-deposit-to-dex-account.js --deposits='[{ "tokenId": "bar", "amount": 100000000 }, { "tokenId": "foo", "amount": 100000000 }]'
npx locklift test $NO_TRACE --tests test/12-pair-deposit-liquidity.js --left_token_id 'foo' --right_token_id 'bar' --left_amount '100000' --right_amount '100000' --auto_change 'false' --contract_name='DexPairPrev' --account_contract_name='DexAccountPrev'
...
echo "____________________________________________________________________";
echo "prev stablepair -> stablepair";
npx locklift test $NO_TRACE --tests test/30-install-pair-code-v2.js --contract_name='DexStablePair' --pool_type=2
npx locklift test $NO_TRACE --tests test/35-upgrade-pair.js --left='bar' --right='qwe' --old_contract_name='DexStablePairPrev' --new_contract_name='DexStablePair' --pool_type=2
...
npx locklift test $DEFAULT_PARAMS --tests test/35-upgrade-pool.js --roots='["foo", "bar", "qwe"]' --old_contract_name='DexStablePoolPrev' --new_contract_name='DexStablePool' --pool_type=3
npx locklift test $DEFAULT_PARAMS --tests test/35-upgrade-pool.js --roots='["foo","qwe"]' --old_contract_name='DexStablePoolPrev' --new_contract_name='DexStablePool' --pool_type=3
...
npx locklift test $NO_TRACE --tests test/32-install-token-vault-code.js --contract_name='DexTokenVault'
npx locklift test $NO_TRACE --tests test/38-upgrade-token-vault.js --token="foo" --old_contract_name="DexTokenVault" --new_contract_name="DexTokenVault"

echo "____________________________________________________________________";
echo "vault -> next vault";
npx locklift test $DEFAULT_PARAMS --tests test/upgrade/3-vault-upgrade-test.js

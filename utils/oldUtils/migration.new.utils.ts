...
export const dexVaultMigration = async (
  ...
  logMigrationProcess("DexVault", "dexVaultMigration", "Deploying DexVault...");
...
export const orderFactoryMigration = async (
  ...
  const orderRootPlatformArtifacts =
    await locklift.factory.getContractArtifacts("OrderRootPlatform");
...
export const multiScatterMigration = async (
  ...
  account: Account,
): Promise<Contract<FactorySource["MultiScatter"]>> => {
  const signer = await locklift.keystore.getSigner("0");

...
export class TokenWallet {
  ...
  constructor(
    wallet_contract: Contract<FactorySource["TokenWalletUpgradeable"]>,
    wallet_owner: Account | Address,
    name?: string,
  ) {
  ...
  async transfer(
    ...
    logTestProcessing(
      `${this.name}(${this.address}).transfer()`,
      `amount: ${amount},
          recipient: ${receiver},
    ...
    return await this.contract.methods
      .transfer({
        ...
        remainingGasTo: !(this._owner instanceof Address)
          ? this._owner.address
          : this._owner,
        notify: notify,
        payload: payload,
      ...
      .send({
        ...
        from: !(this._owner instanceof Address)
          ? this._owner.address
          : this._owner,
      });
  }

...
options.fee.beneficiary_numerator =
  options.fee.beneficiary_numerator || '3000000';
...
function calculateBeneficiary(amount, token) {
  ...
  let poolFee = fee
    .times(options.fee.pool_numerator)
    .div(
      new BigNumber(options.fee.pool_numerator).plus(
        options.fee.beneficiary_numerator,
      ),
    )
    .dp(token.decimals, BigNumber.ROUND_UP);

  return fee.minus(poolFee);
}
...
async function dexPairInfo() {
  ...
  const [accumulated_left_fee, accumulated_right_fee] =
    await DexPairFooBar.call({ method: 'getAccumulatedFees', params: {} });
  ...
  if (IS_FOO_LEFT) {
    ...
    bar = new BigNumber(balances.right_balance)
      .shiftedBy(-Constants.tokens.bar.decimals)
      .toString();
    fooFee = new BigNumber(accumulated_left_fee)
      .shiftedBy(-Constants.tokens.foo.decimals)
      .toString();
    barFee = new BigNumber(accumulated_right_fee)
      .shiftedBy(-Constants.tokens.bar.decimals)
      .toString();
  } else {
    ...
    bar = new BigNumber(balances.left_balance)
      .shiftedBy(-Constants.tokens.bar.decimals)
      .toString();
    fooFee = new BigNumber(accumulated_right_fee)
      .shiftedBy(-Constants.tokens.foo.decimals)
      .toString();
    barFee = new BigNumber(accumulated_left_fee)
      .shiftedBy(-Constants.tokens.bar.decimals)
      .toString();
  }
...
describe(`Test beneficiary fee ${options.pair_contract_name}`, async function () {
  ...
  before('Load contracts', async function () {
    ...
    options.fee.threshold[FooRoot.address] = new BigNumber(2)
      .shiftedBy(Constants.tokens.foo.decimals)
      .toString();
    options.fee.threshold[BarRoot.address] = new BigNumber(2)
      .shiftedBy(Constants.tokens.bar.decimals)
      .toString();

    options.fee.referrer_threshold = {};
    options.fee.referrer_threshold[FooRoot.address] = new BigNumber(3)
      .shiftedBy(Constants.tokens.foo.decimals)
      .toString();
    options.fee.referrer_threshold[BarRoot.address] = new BigNumber(3)
      .shiftedBy(Constants.tokens.bar.decimals)
      .toString();
    ...
    DexAccount3 = await locklift.factory.getContract(
      options.account_contract_name,
    );
    ...
    FooWallet3 = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    ...
    BarWallet3 = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    ...
    FooBarLpWallet3 = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    ...
    FooPairWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    BarPairWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    FooBarLpPairWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
  ...
  describe('Configure fee params', async function () {
    it('Set fee params', async function () {
      logger.log('#################################################');
      logger.log(
        `# DexRoot.setPairFeeParams(${FooRoot.address}, ${
          BarRoot.address
        }, ${JSON.stringify(options.fee)}, ${Account1.address})`,
      );

      const feeParamsStart = await DexPairFooBar.call({
        method: 'getFeeParams',
        params: {},
      });
      logger.log(
      ...
      const feeParamsEnd = await DexPairFooBar.call({
        method: 'getFeeParams',
        params: {},
      });
  ...
  describe('DexAccount deposit', async function () {
    it('Add FOO+BAR liquidity (auto_change=true)', async function () {
      ...
      let expectedBeneficiary = calculateBeneficiary(
        expected.step_2_spent,
        Constants.tokens.foo,
      );
      ...
      expect(expectedDexAccount3Foo).to.equal(
        new BigNumber(dexAccount3End.foo)
          .plus(dexPairInfoEnd.fooFee)
          .toString(),
  ...
  describe('Direct deposit', async function () {
    it('Account#2 deposit BAR liquidity', async function () {
      ...
      logger.log(
        `Account#2 balance start: ` +
          `${
            accountStart.foo !== undefined
              ? accountStart.foo + ' FOO'
              : 'FOO (not deployed)'
          }, ` +
          `${
            accountStart.bar !== undefined
              ? accountStart.bar + ' BAR'
              : 'BAR (not deployed)'
          }, ` +
          `${
      ...
      const expected = await DexPairFooBar.call({
        method: 'expectedDepositLiquidity',
        params: {
          left_amount: IS_FOO_LEFT
            ? 0
            : new BigNumber(TOKENS_TO_DEPOSIT)
                .shiftedBy(Constants.tokens.bar.decimals)
                .toString(),
          right_amount: IS_FOO_LEFT
            ? new BigNumber(TOKENS_TO_DEPOSIT)
                .shiftedBy(Constants.tokens.bar.decimals)
                .toString()
            : 0,
      ...
      logger.log(
        `Account#2 balance end: ` +
          `${
            accountEnd.foo !== undefined
              ? accountEnd.foo + ' FOO'
              : 'FOO (not deployed)'
          }, ` +
          `${
            accountEnd.bar !== undefined
              ? accountEnd.bar + ' BAR'
              : 'BAR (not deployed)'
          }, ` +
          `${
      ...
      expect(dexPairInfoEnd.bar.toString()).to.equal(
        expectedDexBar,
        'Wrong DEX Pair BAR balance',
      );
      expect(accountEnd.bar.toString()).to.equal(
        expectedAccountBar,
        'Wrong Account#2 BAR balance',
      );
      ...
      expect(
        new BigNumber(dexAccount3End.bar)
          .plus(dexPairInfoEnd.barFee)
          .toString(),
      ).to.equal(expectedDexAccount3Bar, 'Wrong beneficiary fee');
    });
    it('Account#2 deposit FOO liquidity', async function () {
      ...
      expect(expectedDexFoo).to.equal(
        ...
        'Wrong DEX Pair FOO balance',
      );
      expect(expectedAccountFoo).to.equal(
        accountEnd.foo.toString(),
        'Wrong Account#2 FOO balance',
      );
  ...
  describe('DexAccount exchanges', async function () {
    it('DexAccount#2 exchange FOO to BAR', async function () {
      ...
      logger.log(
        `DexAccount#3 balance start: ${dexAccount3Start.foo} FOO, ${dexAccount3Start.bar} BAR`,
      );
      logger.log(
      ...
      logger.log(
        `DexAccount#3 balance end: ${dexAccount3End.foo} FOO, ${dexAccount3End.bar} BAR`,
      );
      logger.log(
      ...
      let expectedBeneficiary = calculateBeneficiary(
        new BigNumber(TOKENS_TO_EXCHANGE).shiftedBy(
    ...
    it('DexAccount#2 exchange BAR to FOO', async function () {
      ...
      expect(expectedDexAccount3Bar).to.equal(
        new BigNumber(dexAccount3End.bar)
          .plus(dexPairInfoEnd.barFee)
          .toString(),
        'Wrong beneficiary fee',
  ...
  describe('Direct exchanges', async function () {
    it('Account#2 exchange BAR to FOO', async function () {
      ...
      expect(expectedAccountBar).to.equal(
        accountEnd.bar.toString(),
        'Wrong Account#2 BAR balance',
      );
      expect(expectedPairFoo).to.equal(
        dexPairInfoEnd.foo.toString(),
        'Wrong DEXPair FOO balance',
      );
      expect(expectedPairBar).to.equal(
        dexPairInfoEnd.bar.toString(),
        'Wrong DEXPair BAR balance',
      );
      expect(expectedDexAccount3Bar).to.equal(
        new BigNumber(dexAccount3End.bar)
          .plus(dexPairInfoEnd.barFee)
  ...
  describe('Withdraw beneficiary fee', async function () {
    it('Account#3 withdraw fee', async function () {
      ...
      logger.log(
        `DexAccount#3 balance end: ${dexAccount3Start.foo} FOO, ${dexAccount3Start.bar} BAR`,
      );
      logger.log(
        `${dexPairInfoStart.fooFee} FOO FEE, ${dexPairInfoStart.barFee} BAR FEE`,
      );
      ...
      expect(
        new BigNumber(dexAccount3Start.foo)
          .plus(dexPairInfoStart.fooFee)
          .toString(),
      ).to.equal(
        new BigNumber(dexAccount3End.foo)
      ...
      expect(
        new BigNumber(dexAccount3Start.bar)
          .plus(dexPairInfoStart.barFee)
          .toString(),
      ).to.equal(
        new BigNumber(dexAccount3End.bar)

...
describe('OrderTest', () => {
  ...
  before('deploy and load new migrations', async () => {
    ...
    rootTokenBar = await tokenRootMigration('BarRoot', 'BAR', 9, account1);
    rootTokenReceive = await tokenRootMigration('TstRoot', 'TST', 9, account1);
    ...
    const BarPairWalletAddress = (
      await rootTokenBar.methods
        .walletOf({ walletOwner: dexPair.address, answerId: 0 })
        .call()
    ).value0;
    ...
    await locklift.tracing.trace(
      dexAccount.methods
        ...
        .send({
          amount: locklift.utils.toNano(9),
    ...
    RootOrderBar = await orderRootMigration(
      account1,
      factoryOrder,
      rootTokenBar,
    );
    ...
    await locklift.tracing.trace(
      dexAccount.methods
        .depositLiquidity({
          ...
          left_amount: new BigNumber(200)
            .shiftedBy(Constants.tokens.bar.decimals)
            .toString(),
          right_root: rootTokenReceive.address,
          right_amount: new BigNumber(2000)
  ...
  describe('Direct execution Order', async () => {
    it('Check full execution, case 1.1', async () => {
      console.log(`#############################\n`);
      const amount = await barWallet3.methods.balance({ answerId: 0 }).call();
      ...
      const params = {
        ...
        expectedTokenAmount: new BigNumber(TOKENS_TO_EXCHANGE2)
          .shiftedBy(Constants.tokens.tst.decimals)
          .toString(),
        deployWalletValue: locklift.utils.toNano(0.1),
        backPK: 0,
      ...
      console.log(`BarWallet3(${barWallet3.address}).transfer()
                amount: ${new BigNumber(TOKENS_TO_EXCHANGE1)
                  .shiftedBy(Constants.tokens.bar.decimals)
                  .toString()},
      ...
      const pastEvents = await RootOrderBar.getPastEvents({
        filter: (event) => event.event === 'CreateOrder',
      });
      ...
      const payloadLO = await Order.methods
        .buildPayload({
          callbackId: 1,
          deployWalletValue: locklift.utils.toNano(0.1),
        })
      ...
      await tstWallet4.methods
        .transfer({
          amount: new BigNumber(TOKENS_TO_EXCHANGE2_ACC3)
            .shiftedBy(Constants.tokens.tst.decimals)
            .toString(),
      ...
      await tstWallet5.methods
        .transfer({
          amount: new BigNumber(TOKENS_TO_EXCHANGE2_ACC4)
            .shiftedBy(Constants.tokens.tst.decimals)
            .toString(),
          recipient: Order.address,
          deployWalletValue: locklift.utils.toNano(0.1),
    ...
    it('Check partial execution Order, case 2.2', async () => {
      ...
      await tstWallet6.methods
        .transfer({
          amount: new BigNumber(TOKENS_TO_EXCHANGE2_ACC5)
            .shiftedBy(Constants.tokens.tst.decimals)
            .toString(),
          recipient: Order.address,
          deployWalletValue: locklift.utils.toNano(0.1),
      ...
      const expectedAccount6Bar = new BigNumber(balanceBarAcc6Start.token || 0)
        .plus(BigNumber(TOKENS_TO_EXCHANGE1_ACC5))
        .toString();
      const expectedAccount6Tst = new BigNumber(balanceTstAcc6Start.token || 0)
        .minus(BigNumber(TOKENS_TO_EXCHANGE2_ACC5))
        .toString();
    ...
    it('Check partial execution Order, case 2.3', async () => {
      ...
      const expectedAccount5Tst = new BigNumber(balanceTstAcc5Start.token || 0)
        ...
        .toString();
      const expectedAccount6Bar = new BigNumber(balanceBarAcc6Start.token || 0)
        .plus(BigNumber(TOKENS_TO_EXCHANGE1_ACC5 - 5))
        .toString();
      const expectedAccount6Tst = new BigNumber(balanceTstAcc6Start.token || 0)
        .minus(BigNumber(TOKENS_TO_EXCHANGE2_ACC5 - 10))
        .toString();
    ...
    it('Check create order and closed, case 3.1', async () => {
      ...
      expect(stateL0.value0.toString()).to.equal(
    ...
    it('Check create order and closed, case 3.2', async () => {
      ...
      const expectedAccount3Bar = new BigNumber(balanceBarAcc3Start.token || 0)
        .minus(BigNumber(TOKENS_TO_EXCHANGE1 / 2))
        .toString();
      const expectedAccount3Tst = new BigNumber(balanceTstAcc3Start.token || 0)
        .plus(BigNumber(TOKENS_TO_EXCHANGE2_ACC3))
        .toString();
      const expectedAccount4Bar = new BigNumber(balanceBarAcc4Start.token || 0)
        .plus(BigNumber(TOKENS_TO_EXCHANGE1 / 2))
        .toString();
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
        .minus(BigNumber(TOKENS_TO_EXCHANGE2_ACC3))
        .toString();
    ...
    it('Check execution closed order, case 4.1', async () => {
      ...
      await tstWallet4.methods
        .transfer({
          amount: new BigNumber(TOKENS_TO_EXCHANGE2)
            .shiftedBy(Constants.tokens.tst.decimals)
            .toString(),
          recipient: Order.address,
          deployWalletValue: locklift.utils.toNano(0.1),
  ...
  describe('Execution order via DEX', async () => {
    it('Order from backend SUCCESS', async () => {
      ...
      await Order.methods
        .backendSwap({ callbackId: 1 })
        .sendExternal({ publicKey: signer3.publicKey });
    ...
    it('Order from user SUCCESS', async () => {
      ...
      await sleep(10000);
      const stateLO2 = await Order.methods
        .currentStatus({ answerId: 1 })
        .call();
      ...
      const expectedAccount4Tst = new BigNumber(balanceTstAcc4Start.token || 0)
        ...
        .toString();
      expect(stateLO2.value0.toString()).to.equal(
  ...
  describe('Emergency mode', async () => {
    it('Emergency mode on, send TIP3, off', async () => {
      ...
      const stateLO1 = await Order.methods
        .currentStatus({ answerId: 1 })
        .call();
      expect(stateLO1.value0.toString()).to.equal(
      ...
      await Order.methods
        .proxyTokensTransfer({
          ...
          _amount: new BigNumber(TOKENS_TO_EXCHANGE1)
            .shiftedBy(Constants.tokens.bar.decimals)
            .toString(),
...
async function displayLog(balanceBar, balanceTst, start, accountText) {
  console.log(
    `${accountText} balance ${start == true ? ' start: ' : ' end: '}` +

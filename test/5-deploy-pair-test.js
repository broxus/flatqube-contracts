...
console.log('5-deploy-pair-test.js');
...
let keyPairs, account2, dexRoot, gasValues;
...
describe('Check DexAccount add Pair', async function () {
    ...
    describe('Deploy pairs', async function () {
        for (const p of pairs) {
            ...
            it(`Deploy pair DexPair${pair.left}${pair.right}`, async function () {
                migration.balancesCheckpoint();
                ...
                logger.log('FooBarLpPairWallet: ' + FooBarLpPairWallet.address);
                logger.log('FooPairWallet: ' + FooPairWallet.address);
                logger.log('BarPairWallet: ' + BarPairWallet.address);

                logger.log('FooTokenVault: ' + FooTokenVault.address);
                logger.log('BarTokenVault: ' + BarTokenVault.address);
                logger.log('LpTokenVault: ' + LpTokenVault.address);
                ...
                let checkState = async function (address, name) {
                    let acc_type = -1;
                    await locklift.ton.getAccountType(address).then(v => {
                        acc_type = v.acc_type;
                    }).catch(_ => { /* ignored */ })

                    expect(acc_type)
                        .to
                        .equal(1, name + ' Account not Active');
                }

                await checkState(FooBarLpRoot.address, 'FooBarLpRoot');
                await checkState(FooBarLpPairWallet.address, 'FooBarLpPairWallet');
                await checkState(FooPairWallet.address, 'FooPairWallet');
                await checkState(BarPairWallet.address, 'BarPairWallet');

                await checkState(FooTokenVault.address, 'FooTokenVault');
                await checkState(BarTokenVault.address, 'BarTokenVault');
                await checkState(LpTokenVault.address, 'LpTokenVault');

                await checkState(FooVaultWallet.address, 'FooVaultWallet');
                await checkState(BarTokenVault.address, 'BarTokenVault');
                await checkState(FooBarLpVaultWallet.address, 'FooBarLpVaultWallet');
            });
        }
    })
});

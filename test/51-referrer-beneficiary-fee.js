...
program
  .allowUnknownOption()
  .option(
    '-pcn, --pool_contract_name <pool_contract_name>',
...
function calculateFees(amount, token, fee_numerator) {
  fee_numerator =
    fee_numerator ||
  ...
  let pool = fee
    ...
    .div(
      new BigNumber(options.fee.pool_numerator)
        .plus(options.fee.beneficiary_numerator)
        .plus(options.fee.pool_numerator),
    )
    .dp(token.decimals, BigNumber.ROUND_UP);
  ...
  let beneficiary = fee.minus(pool).minus(referrer);
...
async function dexPoolInfo() {
  const balances_data = await DexPool.call({
    method: 'getBalances',
    params: {},
  });
  let balances = [];
  ...
  const accumulated_fees = await DexPool.call({
    method: 'getAccumulatedFees',
    params: {},
  });
  const tokenBalances = [];
  const tokenFee = [];
...
describe(`Test beneficiary fee ${options.pool_contract_name}`, async function () {
  ...
  before('Load contracts', async function () {
    ...
    for (let i = 0; i < N_COINS; i++) {
      ...
      tokenWallets4[i].setAddress(
        await tokenRoots[i].call({
          method: 'walletOf',
          params: {
  ...
  describe('Configure fee params', async function () {
    it('010 Set fee params', async function () {
      logger.log('#################################################');
      logger.log(
  ...
  describe('DexAccount deposit', async function () {
    it('020 Add FOO+BAR liquidity (auto_change=true)', async function () {
      logger.log('#################################################');
      ...
      if (
        ...
        let fee_numerator = new BigNumber(options.fee.pool_numerator)
          ...
          .dividedBy(4 * (N_COINS - 1));

        for (let i = 0; i < N_COINS; i++) {
          let { beneficiary: expectedBeneficiary, referrer: expectedReferrer } =
            calculateFees(
        ...
        let { beneficiary: expectedBeneficiary, referrer: expectedReferrer } =
          calculateFees(expected.step_2_spent, tokens[0]);
        ...
        logger.log(
          `Referrer fee ${tokens[0].symbol}: ${expectedReferrer.toString()}`,
        );
        ...
        expectedDexAccount3TokenBalances.push(
          dexAccount3Start.accountBalances[1].toString(),
        );

        expectedReferrerBalance = referrerStart.token_balances;
        expectedReferrerBalance[0] = expectedReferrer
  ...
  describe('Direct deposit', async function () {
    it('030 Account#2 deposit Coin2 liquidity', async function () {
      logger.log('#################################################');
      ...
      if (options.pool_contract_name === 'DexPair') {
        ...
        let { beneficiary: expectedBeneficiary, referrer: expectedReferrer } =
          calculateFees(expected.step_2_spent, tokens[1]);
        ...
        logger.log(
          `Referrer fee ${tokens[1].symbol}: ${expectedReferrer.toString()}`,
        );
        ...
        for (let i = 0; i < N_COINS; i++) {
          let { beneficiary: expectedBeneficiary, referrer: expectedReferrer } =
            calculateFees(expected.differences[i], tokens[i], fee_numerator);
        ...
        let { beneficiary: expectedBeneficiary, referrer: expectedReferrer } =
          calculateFees(amounts[1], tokens[1]);
  ...
  describe('Direct exchanges', async function () {
    it(`040 Account#2 exchange Coin2 to Coin1`, async function () {
      logger.log('#################################################');
      logger.log(
      ...
      let { beneficiary: expectedBeneficiary, referrer: expectedReferrer } =
        calculateFees(
          new BigNumber(TOKENS_TO_EXCHANGE).shiftedBy(tokens[1].decimals),
          tokens[1],
        );
    ...
    it('Account#2 exchange Coin1 to Coin2 (expectedSpendAmount)', async function () {
      ...
      let { beneficiary: expectedBeneficiary, referrer: expectedReferrer } =
        calculateFees(expected.expected_amount, tokens[0]);
  ...
  describe('Withdraw beneficiary fee', async function () {
    it('050 Account#3 withdraw fee', async function () {
      logger.log('#################################################');

...
async function account3balances() {
  let foo;
  await FooWallet3.call({ method: 'balance', params: {} })
    .then((n) => {
      foo = new BigNumber(n)
        .shiftedBy(-Constants.tokens.foo.decimals)
        .toString();
  ...
  let bar;
  await BarWallet3.call({ method: 'balance', params: {} })
    .then((n) => {
      bar = new BigNumber(n)
        .shiftedBy(-Constants.tokens.bar.decimals)
        .toString();
  ...
  let lp;
  await FooBarLpWallet3.call({ method: 'balance', params: {} })
    .then((n) => {
...
function logBalances(header, dex, account3, account2, pair) {
  ...
  logger.log(
    `Account#3 balance ${header}: ` +
      `${
        account3.foo !== undefined
          ? account3.foo + ' FOO'
          : 'FOO (not deployed)'
      }, ` +
      `${
        account3.bar !== undefined
          ? account3.bar + ' BAR'
          : 'BAR (not deployed)'
      }, ` +
      `${
  ...
  logger.log(
    `Account#2 balance ${header}: ` +
      `${
        account2.foo !== undefined
          ? account2.foo + ' FOO'
          : 'FOO (not deployed)'
      }, ` +
      `${
        account2.bar !== undefined
          ? account2.bar + ' BAR'
          : 'BAR (not deployed)'
      }, ` +
      `${
...
describe('Check direct DexPairFooBar operations', async function () {
  ...
  describe('Direct exchange (positive)', async function () {
    it('0010 # Account#3 exchange FOO to BAR (with deploy BarWallet#2)', async function () {
      ...
      const expectedDexBar = new BigNumber(dexStart.bar)
        ...
        .toString();
      const expectedAccountFoo = new BigNumber(accountStart3.foo)
    ...
    it('0020 # Account#3 exchange BAR to FOO', async function () {
      ...
      expect(expectedAccountFoo).to.equal(
        accountEnd2.foo.toString(),
        'Wrong Account#2 FOO balance',
      );
      expect(expectedAccountBar).to.equal(
        accountEnd3.bar.toString(),
    ...
    it('0030 # Account#3 exchange BAR to FOO (expectedSpendAmount)', async function () {
      ...
      const payload = await DexPairFooBar.call({
        ...
        params: {
          _id: 0,
          _deployWalletGrams: locklift.utils.convertCrystal('0.05', 'nano'),
          _expectedAmount: new BigNumber(TOKENS_TO_RECEIVE)
            .shiftedBy(Constants.tokens.foo.decimals)
            .toString(),
          _recipient: Account2.address,
          _referrer: locklift.utils.zeroAddress,
    ...
    it('0040 # Account#3 exchange FOO to BAR (small amount)', async function () {
      ...
      const expectedDexBar = new BigNumber(dexStart.bar)
        ...
        .toString();
      const expectedDexFoo = new BigNumber(dexStart.foo)
        .plus(new BigNumber(AMOUNT).shiftedBy(-Constants.tokens.foo.decimals))
        .toString();
      const expectedAccountBar = new BigNumber(accountStart2.bar)
        ...
        .toString();
      const expectedAccountFoo = new BigNumber(accountStart3.foo)
        .minus(new BigNumber(AMOUNT).shiftedBy(-Constants.tokens.foo.decimals))
        .toString();
  ...
  describe('Direct deposit liquidity (positive)', async function () {
    it('0050 # Account#3 deposit FOO liquidity (small amount)', async function () {
      ...
      FooBarLpWallet2.setAddress(
        await FooBarLpRoot.call({
          method: 'walletOf',
          params: {
            walletOwner: Account2.address,
          },
      ...
      const expectedDexFoo = new BigNumber(dexStart.foo)
        .plus(new BigNumber(1000).shiftedBy(-Constants.tokens.foo.decimals))
        .toString();
      const expectedAccountFoo = new BigNumber(accountStart3.foo)
        .minus(new BigNumber(1000).shiftedBy(-Constants.tokens.foo.decimals))
        .toString();
      const expectedAccountLp = new BigNumber(accountStart2.lp || 0)
        .plus(LP_REWARD)
        .toString();

      expect(pairEnd.lp_supply_actual).to.equal(
        pairEnd.lp_supply,
        'Wrong LP supply',
      );
      expect(expectedDexFoo).to.equal(
    ...
    it('0070 # Account#3 deposit BAR liquidity', async function () {
      ...
      const expectedAccountBar = new BigNumber(accountStart3.bar)
        .minus(TOKENS_TO_DEPOSIT)
        .toString();
      const expectedAccountLp = new BigNumber(accountStart2.lp)
        .plus(LP_REWARD)
        .toString();
  ...
  describe('Direct withdraw liquidity (positive)', async function () {
    it('0080 # Account#3 direct withdraw liquidity (small amount)', async function () {
      ...
      const payload = await DexPairFooBar.call({
        method: 'buildWithdrawLiquidityPayloadV2',
        params: {
          _id: 0,
    ...
    it('0090 # Account#3 direct withdraw liquidity', async function () {
      ...
      const expected = await DexPairFooBar.call({
        method: 'expectedWithdrawLiquidity',
        params: {
          lp_amount: new BigNumber(accountStart3.lp)
            .shiftedBy(Constants.LP_DECIMALS)
            .toString(),
        },
      });
      ...
      tx = await Account3.runTarget({
        ...
        method: 'transfer',
        params: {
          amount: new BigNumber(accountStart3.lp)
            .shiftedBy(Constants.LP_DECIMALS)
            .toString(),
          recipient: DexPairFooBar.address,
          deployWalletValue: 0,
  ...
  describe('Direct exchange (negative)', async function () {
    it('0100 # Account#3 exchange FOO to BAR (empty payload)', async function () {
      ...
      expect(accountStart3.foo).to.equal(
        accountEnd3.foo,
        'Wrong Account#3 FOO balance',
      );
      expect(accountStart2.bar).to.equal(
        accountEnd2.bar,
        'Wrong Account#2 BAR balance',
      );
    });

...
async function dexBalances() {
  ...
  const lp = await poolLpVaultWallet
    .call({ method: 'balance', params: {} })
    .then((n) => {
...
function logBalances(header, dex, account, pool) {
  ...
  logs +=
    `LP SUPPLY (PLAN): ${pool.lp_supply || '0'} LP, ` +
  ...
  for (let i = 0; i < N_COINS; i++) {
    logs +=
      account.token_balances[i] !== undefined
        ? `${account.token_balances[i]} ${tokens[i].symbol}, `
        : `${tokens[i].symbol} (not deployed)}, `;
  }
...
describe(`Check direct DexPool${poolName} operations`, async function () {
  ...
  before('Load contracts', async function () {
    ...
    gasValues = migration.load(
      await locklift.factory.getContract('DexGasValues'),
      'DexGasValues',
    );
    ...
    poolLpVaultWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    poolTokenWallets = [];
    for (let i = 0; i < N_COINS; i++) {
      poolTokenWallets.push(
        await locklift.factory.getContract(
          'TokenWalletUpgradeable',
          TOKEN_CONTRACTS_PATH,
        ),
    ...
    poolLpPoolWallet = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
    ...
    poolLpWallet3 = await locklift.factory.getContract(
      'TokenWalletUpgradeable',
      TOKEN_CONTRACTS_PATH,
    );
  ...
  describe('Direct exchange (positive)', async function () {
    ...
    for (let j = 0; j < N_COINS; j++) {
      ...
      it(`0010 # Account#3 exchange Coin_1 to Coin_2 (with deploy Coin_2_Wallet#3)`, async function () {
        logger.log('#################################################');
        logger.log(
          `# Account#3 exchange ${tokens[i].symbol} to ${tokens[j].symbol} (with deploy ${tokens[j].symbol}Wallet#3)`,
        );
        ...
        logger.log(
          `Expected fee: ${new BigNumber(expected.expected_fee)
            .shiftedBy(-tokens[i].decimals)
            .toString()} ${tokens[i].symbol}`,
        );
        logger.log(
          `Expected receive amount: ${new BigNumber(expected.expected_amount)
            .shiftedBy(-tokens[j].decimals)
            .toString()} ${tokens[j].symbol}`,
        );
        ...
        tokenWallets3[j].setAddress(
          await tokenRoots[j].call({
            method: 'walletOf',
            params: {
              walletOwner: Account3.address,
            },
        ...
        const expectedDexSpentToken = new BigNumber(dexStart.token_balances[i])
          .plus(TOKENS_TO_EXCHANGE)
          .toString();
        const expectedDexReceivedToken = new BigNumber(
          dexStart.token_balances[j],
        )
          .minus(
          ...
          .toString();
        const expectedAccountSpentToken = new BigNumber(
          accountStart.token_balances[i],
        )
          .minus(TOKENS_TO_EXCHANGE)
          .toString();
        const expectedAccountReceivedToken = new BigNumber(
          accountStart.token_balances[j] || 0,
        )
          .plus(
            new BigNumber(expected.expected_amount).shiftedBy(
              -tokens[j].decimals,
        ...
        expect(expectedDexSpentToken).to.equal(
          dexEnd.token_balances[i].toString(),
          `Wrong DEX ${tokens[i].symbol} balance`,
        );
        expect(expectedDexReceivedToken).to.equal(
          dexEnd.token_balances[j].toString(),
          `Wrong DEX ${tokens[j].symbol} balance`,
        );
        expect(expectedAccountSpentToken).to.equal(
          accountEnd.token_balances[i].toString(),
          `Wrong DexAccount#3 ${tokens[i].symbol} balance`,
        );
        expect(expectedAccountReceivedToken).to.equal(
          accountEnd.token_balances[j].toString(),
          `Wrong DexAccount#3 ${tokens[j].symbol} balance`,
        );
      });
    }
    ...
    for (let i = 0; i < N_COINS; i++) {
      ...
      it('0020 # Account#3 exchange Coin_1 to Coin_2', async function () {
        logger.log('#################################################');
        logger.log(
          `# Account#3 exchange ${tokens[i].symbol} to ${tokens[j].symbol}`,
        );
        const dexStart = await dexBalances();
        const accountStart = await account3balances();
        const poolStart = await dexPoolInfo();
        ...
        logger.log(
          `Spent amount: ${TOKENS_TO_EXCHANGE.toString()} ${tokens[i].symbol}`,
        );
        logger.log(
          `Expected fee: ${new BigNumber(expected.expected_fee)
            .shiftedBy(-tokens[i].decimals)
      ...
      it('0030 # Account#3 exchange Coin1 to Coin2 (expectedSpendAmount)', async function () {
        ...
        logger.log(
          `Expected receive amount: ${TOKENS_TO_RECEIVE} ${tokens[j].symbol}`,
        );
        ...
        const expectedDexReceivedAmount = new BigNumber(
          dexStart.token_balances[j],
        )
        ...
        const expectedDexSpentAmount = new BigNumber(dexStart.token_balances[i])
          ...
          .toString();
        const expectedAccountReceivedAmount = new BigNumber(
          accountStart.token_balances[j],
        )
          .plus(TOKENS_TO_RECEIVE)
          .toString();
        const expectedAccountSpentAmount = new BigNumber(
          accountStart.token_balances[i],
        )
          .minus(
            new BigNumber(expected.expected_amount).shiftedBy(
        ...
        expect(expectedDexReceivedAmount).to.equal(
          dexEnd.token_balances[j].toString(),
          `Wrong DEX ${tokens[j].symbol} balance`,
        );
        expect(expectedDexSpentAmount).to.equal(
          dexEnd.token_balances[i].toString(),
          `Wrong DEX ${tokens[i].symbol} balance`,
        );
        expect(expectedAccountReceivedAmount).to.equal(
          accountEnd.token_balances[j].toString(),
          `Wrong Account#3 ${tokens[j].symbol} balance`,
        );
        expect(expectedAccountSpentAmount).to.equal(
          accountEnd.token_balances[i].toString(),
    ...
    it('0040 # Account#3 exchange Coin_1 to Coin_2 (small amount)', async function () {
      ...
      logger.log('#################################################');
      logger.log(
        `# Account#3 exchange ${tokens[i].symbol} to ${tokens[j].symbol} (small amount)`,
      );
      const dexStart = await dexBalances();
      const accountStart = await account3balances();
      const poolStart = await dexPoolInfo();
      ...
      const AMOUNT =
        100 *
        Math.pow(10, Math.max(tokens[i].decimals - tokens[j].decimals, 0));
      ...
      const expectedDexReceivedAmount = new BigNumber(
        ...
        .toString();
      const expectedDexSpentAmount = new BigNumber(dexStart.token_balances[i])
        .plus(new BigNumber(AMOUNT).shiftedBy(-tokens[i].decimals))
        .toString();
      const expectedAccountReceivedAmount = new BigNumber(
        accountStart.token_balances[j],
      )
      ...
      const expectedAccountSpentAmount = new BigNumber(
        ...
        .minus(new BigNumber(AMOUNT).shiftedBy(-tokens[i].decimals))
        .toString();
      expect(expectedDexSpentAmount).to.equal(
        dexEnd.token_balances[i].toString(),
        `Wrong DEX ${tokens[i].symbol} balance`,
  ...
  describe('Direct deposit liquidity (positive)', async function () {
    it('0050 # Account#3 deposit single coin liquidity (small amount)', async function () {
      ...
      logger.log('#################################################');
      logger.log(
        `# Account#3 deposit ${tokens[i].symbol} liquidity (small amount)`,
      );
      const dexStart = await dexBalances();
      const accountStart = await account3balances();
      const poolStart = await dexPoolInfo();
      ...
      let amount =
        300 *
        Math.pow(10, Math.max(tokens[i].decimals - Constants.LP_DECIMALS, 0));
      ...
      poolLpWallet3.setAddress(
        await poolLpRoot.call({
          method: 'walletOf',
          params: {
            walletOwner: Account3.address,
          },
      ...
      const expectedDexCoin = new BigNumber(dexStart.token_balances[i])
        .plus(new BigNumber(amount).shiftedBy(-tokens[i].decimals))
        .toString();
      const expectedAccountCoin = new BigNumber(accountStart.token_balances[i])
        .minus(new BigNumber(amount).shiftedBy(-tokens[i].decimals))
        .toString();
      ...
      expect(poolEnd.lp_supply_actual).to.equal(
        poolEnd.lp_supply,
        'Wrong LP supply',
      );
      expect(expectedDexCoin).to.equal(
        dexEnd.token_balances[i].toString(),
        `Wrong DEX ${tokens[i].symbol} balance`,
      );
      expect(expectedAccountCoin).to.equal(
        accountEnd.token_balances[i].toString(),
        `Wrong Account#3 ${tokens[i].symbol} balance`,
      );
      expect(expectedAccountLp).to.equal(
        accountEnd.lp.toString(),
    ...
    for (let i = 0; i < N_COINS; i++) {
      it('0060 # Account#3 deposit single coin liquidity', async function () {
        ...
        let amount = new BigNumber(TOKENS_TO_DEPOSIT)
          .shiftedBy(tokens[i].decimals)
          .toString();
        ...
        const expectedDexCoin = new BigNumber(dexStart.token_balances[i])
          ...
          .toString();
        const expectedAccountCoin = new BigNumber(
          accountStart.token_balances[i],
        )
      ...
      it('0030 # Account#3 direct deposit single coin liquidity (expectedSpendAmount)', async function () {
        ...
        const expected = await DexPool.call({
          method: 'expectedDepositSpendAmount',
          params: {
            lp_amount: new BigNumber(LP_TO_RECEIVE)
              .shiftedBy(Constants.LP_DECIMALS)
              .toString(),
            spent_token_root: tokenRoots[i].address,
          },
        ...
        logger.log(
          `Expected spend amount: ${new BigNumber(expected.tokens_amount)
            .shiftedBy(-tokens[i].decimals)
            .toString()} ${tokens[i].symbol}`,
        );
        logger.log(
        ...
        const expectedDexSpentAmount = new BigNumber(dexStart.token_balances[i])
          .plus(
            new BigNumber(expected.tokens_amount).shiftedBy(
              -tokens[i].decimals,
            ),
          )
          .toString();
        ...
        expect(new BigNumber(poolEnd.lp_supply).toNumber()).to.be.gte(
          expectedPoolLp,
          `Wrong DEX LP balance`,
        );
        expect(new BigNumber(accountEnd.lp).toNumber()).to.be.gte(
          expectedAccountReceivedAmount,
          `Wrong Account#3 LP balance`,
        );
      });
    }
    ...
    it('0080 # Account#3 direct withdraw liquidity (small amount)', async function () {
      ...
      for (let i = 0; i < N_COINS; i++) {
        expectedDexAmount.push(
          new BigNumber(dexStart.token_balances[i])
            .minus(expectedAmount[i])
            .toString(),
        );
      }
      ...
      for (let i = 0; i < N_COINS; i++) {
        expectedAccountAmount.push(
          new BigNumber(accountStart.token_balances[i])
            .plus(expectedAmount[i])
            .toString(),
        );
      }
      ...
      for (let i = 0; i < N_COINS; i++) {
        expect(expectedDexAmount[i]).to.equal(
          dexEnd.token_balances[i].toString(),
          `Wrong DEX ${tokens[i].symbol} balance`,
        );
      }
      for (let i = 0; i < N_COINS; i++) {
        expect(expectedAccountAmount[i]).to.equal(
          accountEnd.token_balances[i].toString(),
          `Wrong Account#3 ${tokens[i].symbol} balance`,
        );
      }
  ...
  describe('Direct withdraw single coin liquidity (positive)', async function () {
    it('0080 # Account#3 direct withdraw single coin liquidity (small amount)', async function () {
      ...
      logger.log('#################################################');
      logger.log(
        `# Account#3 direct withdraw ${tokens[i].symbol} liquidity (small amount)`,
      );
      const dexStart = await dexBalances();
      const accountStart = await account3balances();
      const poolStart = await dexPoolInfo();
      ...
      const LP_AMOUNT =
        1000 *
        Math.pow(10, Math.max(0, Constants.LP_DECIMALS - tokens[i].decimals));
      ...
      logger.log(
        'Lp amount: ',
        new BigNumber(LP_AMOUNT).shiftedBy(-Constants.LP_DECIMALS).toString(),
      );
      logger.log(
        `Expected amount: ${new BigNumber(expected.amounts[i])
          .shiftedBy(-tokens[i].decimals)
          .toString()} ${tokens[i].symbol}`,
      );

      const payload = await DexPool.call({
        method: 'buildWithdrawLiquidityOneCoinPayload',
        params: {
          id: 0,
          deploy_wallet_grams: 0,
      ...
      const expectedDexAmount = new BigNumber(dexStart.token_balances[i])
        .minus(
          new BigNumber(expected.amounts[i]).shiftedBy(-tokens[i].decimals),
        )
        .toString();
      const expectedAccountAmount = new BigNumber(
        accountStart.token_balances[i],
      )
        .plus(new BigNumber(expected.amounts[i]).shiftedBy(-tokens[i].decimals))
        .toString();
      ...
      expect(expectedDexAmount).to.equal(
        dexEnd.token_balances[i].toString(),
        `Wrong DEX ${tokens[i].symbol} balance`,
      );
      expect(expectedAccountAmount).to.equal(
        accountEnd.token_balances[i].toString(),
        `Wrong Account#3 ${tokens[i].symbol} balance`,
      );
    });
    ...
    for (let i = 0; i < N_COINS; i++) {
      ...
      it('0030 # Account#3 direct withdraw single coin liquidity (expectedSpendAmount)', async function () {
        ...
        const expected = await DexPool.call({
          method: 'expectedOneCoinWithdrawalSpendAmount',
          params: {
            receive_amount: new BigNumber(TOKENS_TO_RECEIVE)
              .shiftedBy(tokens[i].decimals)
              .toString(),
        ...
        logger.log(
          `Expected spend amount: ${new BigNumber(expected.lp)
            .shiftedBy(-Constants.LP_DECIMALS)
            .toString()} LP`,
        );
        logger.log(
        ...
        logger.log(
          `Expected receive amount: ${TOKENS_TO_RECEIVE} ${tokens[i].symbol}`,
        );
        ...
        const expectedDexReceivedAmount = new BigNumber(
          dexStart.token_balances[i],
        )
          .minus(TOKENS_TO_RECEIVE)
          .toNumber();
        const expectedPoolSpentAmount = new BigNumber(poolStart.lp_supply)
          .minus(new BigNumber(expected.lp).shiftedBy(-Constants.LP_DECIMALS))
          .toString();
        const expectedAccountReceivedAmount = new BigNumber(
          accountStart.token_balances[i],
        )
        ...
        expect(
          new BigNumber(expectedDexReceivedAmount).toNumber(),
        ).to.approximately(
          new BigNumber(dexEnd.token_balances[i]).toNumber(),
          new BigNumber(1).shiftedBy(-Constants.LP_DECIMALS).toNumber(),
          `Wrong DEX ${tokens[i].symbol} balance`,
        );
        expect(expectedPoolSpentAmount).to.equal(
          poolEnd.lp_supply.toString(),
          `Wrong DEX LP balance`,
        );
        expect(
          new BigNumber(expectedAccountReceivedAmount).toNumber(),
        ).to.approximately(
        ...
        expect(
          new BigNumber(expectedAccountSpentAmount).toNumber(),
        ).to.approximately(
  ...
  describe('Direct exchange (negative)', async function () {
    ...
    it('0100 # Account#3 exchange Coin_1 to Coin_2 (empty payload)', async function () {
      logger.log('#################################################');
      logger.log(
        `# Account#3 exchange ${tokens[i].symbol} to ${tokens[j].symbol} (empty payload)`,
      );
      const dexStart = await dexBalances();
      const accountStart = await account3balances();
      const poolStart = await dexPoolInfo();
      ...
      expect(dexStart.token_balances[i]).to.equal(
        dexEnd.token_balances[i],
        `Wrong DEX ${tokens[i].symbol} balance`,
      );
      expect(dexStart.token_balances[j]).to.equal(
        dexEnd.token_balances[j],
        `Wrong DEX ${tokens[j].symbol} balance`,
      );
      expect(accountStart.token_balances[i]).to.equal(
        accountEnd.token_balances[i],
        `Wrong Account#3 ${tokens[i].symbol} balance`,
      );
      expect(accountStart.token_balances[j]).to.equal(
        accountEnd.token_balances[j],
        `Wrong Account#3 ${tokens[j].symbol} balance`,
      );
    });
    ...
    it('0110 # Account#3 exchange Coin_1 to Coin_2 (low gas)', async function () {
      logger.log('#################################################');
      logger.log(
        `# Account#3 exchange ${tokens[i].symbol} to ${tokens[j].symbol} (low gas)`,
      );
      const dexStart = await dexBalances();
      const accountStart = await account3balances();
      const poolStart = await dexPoolInfo();
    ...
    it('0120 # Account#3 exchange Coin_1 to Coin_2 (wrong rate)', async function () {
      logger.log('#################################################');
      logger.log(
        `# Account#3 exchange ${tokens[i].symbol} to ${tokens[j].symbol} (wrong rate)`,
      );
      const dexStart = await dexBalances();
      const accountStart = await account3balances();
      const poolStart = await dexPoolInfo();
